...
ChatLogic::~ChatLogic()
{
  ...
  // delete all nodes --> We do not need the deletion, since the new implementation does use smartpointer that delete the elements automatically when the destructor is called (if the Class ChatLogic goes out of scope)
  /*
          ...
          if (type->second == "NODE")
          {
            ...
            auto newNode = std::find_if(
                ...
                [&id](std::unique_ptr<GraphNode> &node){
                  // We need to pass an unique_ptr reference since we do not want to hand over ownership to the lambda function but there is no possability to hand over the raw pointer since we use a STL function. To hand over a raw pointer we would need to edit the STL implementation 
                  return node->GetID() == id; 
                  }); // this is a lambda function, [&id] makes the id avalable to the labmda function by reference
                // std::find_if returns the first element that satisfies the condition function (here this is the lambda function)
                // No change of the (lambda-)function parameter, since the lambda just needs the value and not owner rights
