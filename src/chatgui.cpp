...
bool ChatBotApp::OnInit() {
  ...
  /*
   * CAUTION: The chatBotFrame is allocated on the heap and no delete is called
   * afterwards, this is a potential memory leak for the programm. Since
   * OnInit() is the entry point, which is only called once and chatBotFrame
   * must exist during the complete runtime of the programm, there is no need to
   * delete the memory, because the memory is freed automatically when the
   * program exits.
   */
...
void ChatBotFrame::OnEnter(wxCommandEvent &WXUNUSED(event)) {
  ...
  _panelDialog->GetChatLogicHandle()->SendMessageToChatbot(
      std::string(userText.mb_str())); // To let the chatbot know, what the user
                                       // wants to ask him
}
...
ChatBotPanelDialog::ChatBotPanelDialog(wxWindow *parent, wxWindowID id)
  ...
  _chatLogic = std::unique_ptr<ChatLogic>(
      new ChatLogic()); // Task 1: Make this a unique_ptr to manage the
                        // ChatLogic exclusivly by the ChatBotPanelDialog
...
ChatBotPanelDialog::~ChatBotPanelDialog() {
  //// STUDENT CODE
  ////
  // delete _chatLogic; // ne delete arugment needed, since the unique_ptr
  // deallocates the memory as soon as the ChatbotPanelDialog instance goes out
  // of scope
